...
function $updateBlogSequence(id, sequene) {
  return new Promise((resolve, reject) => {
    BlogSequence.findOneAndUpdate({ _id: id }, { sequene }, (error, _sequene) => {
      if (error) { return reject(error); }
      return resolve(_sequene);
    });
  });
}

function $retrieveBlogsBySequence(sequence) {
  return new Promise((resolve, reject) => {
    Blog.find({ _id: { $in: sequence } }, (error, _blogSequenceList) => {
      if (error) { return reject(error); }
      const $blogSequenceList = _blogSequenceList.map((blog) => {
        const $blog = blog;
        return { _id: $blog._id, introduce: $blog.introduce, title: $blog.title };
      });
      return resolve($blogSequenceList);
    });
  });
}
